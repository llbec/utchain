...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "UlordNetwork") type = UlordNetwork;
    if(strType == "UlordNetworkVariable") type = UlordNetworkVariable;
    ...
    if(strType == "Socialgovern") type = Contract;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == UlordNetwork) s = "UlordNetwork";
    if(type == UlordNetworkVariable) s = "UlordNetworkVariable";
