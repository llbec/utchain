...
const char * const BITCOIN_CONF_FILENAME = "ulord.conf";
const char * const BITCOIN_PID_FILENAME = "ulordd.pid";
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UlordCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/UlordCore";
#else
    // Unix
    return pathRet / ".ulordcore";
#endif
#endif
}
		   ...
		   && !isend(p[i]) ) {
			...
			if(';' != p[j]) //skip over comment
			{
				while(j < i && p[j] != '=') {
					j++;
					if('=' == p[j]) {
						if((int)strlen(key)==(j-newline_start) && strncmp(key,p+newline_start,j-newline_start)==0)
						{
							//find the key ok
							*key_s = newline_start;
							*key_e = j-1;
...
{
    return write_profile_string_nosection(key.c_str(), value.c_str(), file.c_str());
}
...
int write_profile_string_nosection( const char *key, const char *value, const char *file)
{
	...
	int sec_s = 0, key_s = 0, key_e = 0, value_s = 0, value_e = 0;
